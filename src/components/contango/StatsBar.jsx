...
export default function StatsBar({ compact = false }) {
  ...
  tiles.push(<StatTile key="hearts" icon={<Heart className={`h-3.5 w-3.5 ${depleted ? "fill-rose-400" : "fill-rose-400/20"}`} />} value={`${progress.hearts}/${MAX_HEARTS}`} label="hearts" color="text-rose-400" bg={depleted ? "bg-rose-500/20" : "bg-rose-500/10"} border={depleted ? "border-rose-500/40" : "border-slate-800/60"} />);
...
function StatTile({ icon, value, label, color, bg, border = "border-slate-800/60" }) {
  return (
    <div
      title={label}
